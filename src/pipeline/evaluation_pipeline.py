...
class EvaluationPipeline(Pipeline):
    ...
    def run_agent(self, agent: BaseAgent, model_interaction_budget: int, run_idx: int, overwrite: bool, name: str, hints: str = "", grader_num_repeat: int = 1) -> Dict[str, Any]:
        ...
        async def grade_all():
            tasks = [
                grade_and_save_async(self.cfg, description, save_dir=out_dir, run_id=grader_run_idx)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for model_interaction_budget in agent_cfg.budgets.model_interactions:
            ...
            for run_idx in range(num_repeat):
                ...
                agent_score, _agent_text, description = self.run_agent(agent, model_interaction_budget, run_idx, overwrite, agent.name + "_" + name, hints=agent_cfg.hints, grader_num_repeat=grader_num_repeat)
        ...
        if agent_cfg.baselines.enabled:
            for model_interaction_budget in agent_cfg.baselines.budgets.model_interactions:
                for run_idx in range(num_repeat):
                    ...
                    baseline_score, _baseline_text, description = self.run_agent(baseline, model_interaction_budget, run_idx, overwrite, baseline.name + "_" + name, hints=agent_cfg.hints, grader_num_repeat=grader_num_repeat)