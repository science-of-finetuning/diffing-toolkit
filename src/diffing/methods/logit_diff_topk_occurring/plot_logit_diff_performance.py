...
AGENT_LLM_FILTER: Optional[str] = "openai/gpt-5"  # Match LogitDiff runs


# Variants to visualize and their display properties
VARIANTS: List[Tuple[str, str]] = [
    ("agent_mi0", "LogitDiff$^{i=0}$"),
    ("agent_mi5", "LogitDiff$^{i=5}$"),
...
def _results_root_from_cfg(cfg) -> Path:
    root = Path(cfg.diffing.results_dir) / "logit_diff_topk_occurring"
...
def _find_all_grade_paths_by_kind_and_mi(
    ...
    *,
    ...
    if is_baseline:
        ...
        pat_new_str = r"^Blackbox_.*_mi" + re.escape(str(mi))
        if position is not None:
            pat_new_str += r"_pos" + re.escape(str(position))
        pat_new_str += r"_run\d+$"
        pat_new = re.compile(pat_new_str)
    else:
        # Agent: <AgentType>_<llm>_mi<N>_run<X> (LogitDiff, ADL, etc.)
        pat_new_str = r"^[A-Za-z]+_.*_mi" + re.escape(str(mi))
        if position is not None:
            pat_new_str += r"_pos" + re.escape(str(position))
        pat_new_str += r"_run\d+$"
        pat_new = re.compile(pat_new_str)
    ...
    for child in agent_root.iterdir():
        ...
        if grade_files:
            out.extend(grade_files)
        else:
            # Fall back to old format: hypothesis_grade.json in ours/ or root
            grade_path = (
...
# %%
# LogitDiff Experiment Analysis
# Uncomment the lines below to run the analysis:
# entries_logit_diff = [
