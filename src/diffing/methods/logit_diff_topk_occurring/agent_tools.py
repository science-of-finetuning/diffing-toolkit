        ...
        try:
            ...
            if selection_mode == "fraction_positive_diff":
                ...
                if not global_stats_file.exists():
                    ...
                    logger.info(
                        f"Using fraction_positive_diff mode for {ds} ({len(top_positive)} tokens)"
                    )
            else:
                ...
                if filter_punct or normalize or filter_special:
                    ...
                    logger.info(
                        f"Applied token processing for {ds}: {original_len} -> {len(top_positive)} tokens (filter_punct={filter_punct}, normalize={normalize}, filter_special={filter_special})"
                    )
            ...
            if top_k_tokens is not None:
                ...
                logger.info(
                    f"Limited tokens for agent overview: {original_count} -> {len(top_positive)} tokens (top_k_tokens={top_k_tokens})"
                )
