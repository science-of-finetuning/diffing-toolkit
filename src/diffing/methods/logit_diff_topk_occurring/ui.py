...
def visualize(method):
    ...
    multi_tab_interface(
        [
            (
                "📊 Token Occurrence Rankings",
                lambda: _render_occurrence_rankings_tab(method),
            ),
...
def _render_global_scatter_tab(method):
    ...
    selected_dataset = st.selectbox(
        "Select Dataset", available_datasets, key="scatter_dataset_select"
    )
    ...
    if search_text:
        ...
        try:
            ...
            if matched_count > 0:
                ...
                st.warning(
                    "Tokens found in tokenizer but not present in the analysis stats (dataset might be too small)."
                )
...
def _render_occurrence_rankings_tab(method):
    ...
    if filter_punct and normalize:
        st.info(
            "📝 Filtering pure punctuation AND normalizing tokens (lowercase, consolidate)"
        )
    elif filter_punct:
...
def _render_interactive_heatmap_tab(method):
    ...
    st.markdown(
        "Enter custom text to analyze logit differences between base and finetuned models."
    )
...
def _prepare_heatmap_data(
    ...
    for pos in range(seq_len):
        ...
        diff_neg_values, diff_neg_indices = torch.topk(
            diff[pos], k=k_half, largest=False
        )
        diff_top_k_negative = [
            {"token": tokenizer.decode([idx.item()]), "diff": val.item()}
...
def _plot_heatmap(
    ...
    def render_cell(
        ...
        rect = mpatches.Rectangle(
            (x, y), width, height, facecolor=color, edgecolor="white", linewidth=0.5
        )
        ax.add_patch(rect)
    ...
    norm_all_diffs = (
        normalize_values([abs(d) for d in all_diffs]) if all_diffs else np.array([])
    )
    ...
    ax.text(
        -0.3,
        current_row + (k_half * cell_height / 2),
        "Diff -\n(M1>M2)",
        ha="right",
        va="center",
        fontsize=font_sizes["heatmap_labels"],
        weight="bold",
        color="darkred",
    )
    ...
    num_model1_logits = len(
        [item for pos_data in sample_data for item in pos_data["model1_top_k"][:k]]
    )
    ...
    for pos_idx_local, pos_data in enumerate(sample_data):
        ...
        for i, logit_item in enumerate(
            pos_data["model2_top_k"][:k]
        ):  # Only take first k items
    ...
    ax.text(
        -0.3,
        current_row + (position_row_height / 2),
        "Position",
        ha="right",
        va="center",
        fontsize=font_sizes["heatmap_labels"],
    ...
    ax.text(
        -0.3,
        current_row + (reference_row_height / 2),
        "Reference\nTokens",
        ha="right",
        va="center",
        fontsize=font_sizes["heatmap_labels"],
