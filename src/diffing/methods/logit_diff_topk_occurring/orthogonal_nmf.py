...
def fit_nmf_orthogonal(
    ...
    if verbose:
        logger.info(f"Starting orthogonal NMF: {V.shape[0]} samples x {V.shape[1]} tokens -> {rank} topics")
        logger.info(f"  beta={beta}, orthogonal_weight={orthogonal_weight}, max_iter={max_iter}")
    ...
    for iteration in range(max_iter):
        ...
        if verbose and (iteration + 1) % 50 == 0:
            ...
            logger.info(f"  Iteration {iteration + 1}/{max_iter}, reconstruction error: {error:.4f}")
