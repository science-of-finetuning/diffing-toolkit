...
matplotlib.rcParams["text.antialiased"] = (
    True  # Always enable anti-aliasing for smooth text
)
...
def plot_occurrence_bar_chart(
    ...
    fig, (ax_neg, ax_pos) = plt.subplots(
        1, 2, figsize=(figure_width, figure_height), dpi=figure_dpi
    )
    ...
    ax_neg.barh(
        y_neg, neg_values, color="red", alpha=0.7, edgecolor="darkred", linewidth=0.5
    )
...
def plot_co_occurrence_heatmap(
    ...
    if n_tokens == 0:
        logger.warning(
            f"No tokens found for co-occurrence analysis ({co_occurrence_type})"
        )
        return None
    ...
    df_norm = pd.DataFrame(
        normalized_data, index=wrapped_tokens, columns=wrapped_tokens
    )
    ...
    plt.title(
        f"Token Co-occurrence: {title_suffix}\nDataset: {dataset_name}",
        fontsize=14,
        pad=20,
    )
...
class TricolorRectangleHandler(HandlerPatch):
    ...
    def create_artists(
        ...
        green_rect = mpatches.Rectangle(
            [xdescent + red_width + gray_width, ydescent],
            green_width,
            height,
            facecolor="tab:green",
            edgecolor="white",
...
def plot_shortlist_token_distribution(
    ...
    if num_samples > 0:
        subtitle_lines.append(
            f"Samples: {num_samples} | Max Pos: {max_tokens_per_sample} | Total Pos: {total_positions}"
        )

    subtitle_lines.append(
        f"Positive: {count_positive} ({fraction_positive:.2f}) | Avg Diff: {avg_logit_diff:.2f}"
    )
...
def plot_shortlist_token_distribution_by_position(
    ...
    sorted_positions = sorted(
        [p for p in logit_diffs_by_position.keys() if p < num_positions]
    )
...
def plot_shortlist_token_distribution_by_sample(
    ...
    sorted_samples = sorted(
        [s for s in logit_diffs_by_sample.keys() if s < num_samples]
    )
...
def plot_global_token_scatter(
    ...
    if filter_punctuation and filtered_count > 0:
        logger.info(
            f"Filtered {filtered_count} pure punctuation/whitespace tokens from scatter plot"
        )
    ...
    if top_k_labels is not None and top_k_labels > 0:
        if (
            occurrence_rates_json_path is not None
            and occurrence_rates_json_path.exists()
        ):
    ...
    if top_k_labels is not None and top_k_labels > 0:
        ...
        for idx in annotate_indices:
            ...
            texts.append(
                plt.text(
                    x_coords[idx], y_coords[idx], display_str, fontsize=6, color="black"
                )
            )
    ...
    suffix = (
        "" if (top_k_labels is not None and top_k_labels > 0) else "_no_text_labels"
    )
...
def get_global_token_scatter_plotly(
    ...
    if filter_punctuation and filtered_count > 0:
        logger.info(
            f"Filtered {filtered_count} pure punctuation/whitespace tokens from Plotly scatter"
        )
...
def plot_positional_kde(
    ...
    sorted_positions = sorted(
        [p for p in position_logit_diffs.keys() if p < num_positions]
    )

    if not sorted_positions:
        logger.warning(
            f"No data found for positions 0-{num_positions-1} in {dataset_name}"
        )
        return None
    ...
    if not data_records:
        logger.warning(
            f"No valid values found for positional KDE plot in {dataset_name}"
        )
        return None
...
def plot_pairwise_token_correlation(
    ...
    assert len(x) == len(
        y
    ), f"Token diff lists must be same length: {len(x)} vs {len(y)}"
    ...
    hexbin = ax.hexbin(
        x, y, gridsize=50, cmap="viridis", mincnt=1, alpha=0.8, linewidths=0.1
    )
