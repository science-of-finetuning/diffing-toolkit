...
class LogitDiffTopKOccurringMethod(DiffingMethod):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.logit_type = str(
            getattr(self.method_cfg.method_params, "logit_type", "direct")
        )
        ...
        if self.logit_type == "adl_logitlens":
            ...
            self.logger.info(
                f"ADL LogitLens mode: using layer {self.adl_layer_relative} (absolute: {self.adl_layer_idx})"
            )
        elif self.logit_type != "direct":
            raise ValueError(
                f"Unknown logit_type: '{self.logit_type}'. Expected 'direct' or 'adl_logitlens'."
            )
    ...
    def get_or_create_results_dir(self) -> Path:
        ...
        analysis_folders = sorted(
            [
                d
                for d in self.base_results_dir.iterdir()
    ...
    def _prepare_dataset_tensors(
        ...
        if dataset_cfg.is_chat:
            ...
            all_positions = [
                sample["positions"] for sample in samples
            ]  # Extract positions for slicing
        else:
    ...
    def compute_stats_from_logits(
        ...
        if max_samples > available_samples:
            ...
            self.logger.info(
                f"Using first {max_samples} samples (have {available_samples})"
            )
        ...
        if max_tokens > available_positions:
            ...
            self.logger.info(
                f"Using first {max_tokens} positions (have {available_positions})"
            )
        ...
        global_token_counts = defaultdict(
            lambda: {"count_positive": 0, "count_negative": 0}
        )
        total_positions = 0
        ...
        if (
            hasattr(self.method_cfg, "positional_kde")
            and self.method_cfg.positional_kde.enabled
        ):
        ...
        if (
            hasattr(self.method_cfg, "per_token_analysis")
            and self.method_cfg.per_token_analysis.enabled
        ):
            ...
            if (
                hasattr(self.method_cfg.per_token_analysis, "co_occurrence")
                and self.method_cfg.per_token_analysis.co_occurrence
            ):
        ...
        self.logger.info(
            f"Processing {num_samples} samples in {num_batches} batches..."
        )
        ...
        global_pos_token_counts = torch.zeros(
            vocab_size, dtype=torch.int64, device="cpu"
        )
        global_neg_token_counts = torch.zeros(
            vocab_size, dtype=torch.int64, device="cpu"
        )
        ...
        vec_same_point_matrix = torch.zeros(
        ...
        for batch_idx in tqdm(
            range(num_batches), desc=f"Processing {dataset_cfg.name}"
        ):
            ...
            if per_token_enabled and shortlist_token_ids:
                ...
                for s_token_id, s_token_str in shortlist_token_ids.items():
                    ...
                    for b_idx in range(batch_size_curr):
                        ...
                        shortlist_diffs_by_sample[s_token_str][
                            sample_idx_global
                        ].extend(vals_for_sample.cpu().tolist())
            ...
            if nmf_enabled:
                ...
                valid_positions_mask = (
                    attention_mask_batch.bool()
                    if ignore_padding
                    else torch.ones_like(attention_mask_batch, dtype=torch.bool)
                )
                ...
                row_indices = torch.arange(
                    row_start, row_start + num_valid_in_batch, device="cpu"
                )
                nmf_data["valid_row_idx_counter"] += num_valid_in_batch

                # Get top-K indices and values for valid positions
                flat_indices = top_k_pos_indices.view(-1, top_k)[
                    valid_flat.cpu()
                ].cpu()  # [num_valid, topk]
                flat_values = top_k_pos_values.view(-1, top_k)[
                    valid_flat.cpu()
                ].cpu()  # [num_valid, topk]
                ...
                for k_idx in range(top_k):
                    ...
                    for row_idx_local, token_id_item in enumerate(token_ids_k):
                        ...
                        if token_id_item not in nmf_data["token_id_to_col_idx"]:
                            nmf_data["token_id_to_col_idx"][token_id_item] = nmf_data[
                                "next_col_idx"
                            ]
        ...
        if co_occurrence_enabled and shortlist_idx_to_str:
            ...
            same_sample_cooc = (
                presence_per_sample.T @ presence_per_sample
            )  # [shortlist, shortlist]
            ...
            same_position_cooc = (
                presence_per_position.T @ presence_per_position
            )  # [shortlist, shortlist]
        ...
        if global_stats_enabled and global_diff_sum is not None:
            ...
            json_path = (
                self.analysis_dir / f"{dataset_cfg.name}_global_token_stats.json"
            )
            ...
            all_tokens.append(
                {
                    ...
                    "positive_occurrence_rate": (
                        (counts["count_positive"] / total_positions) * 100
                        if total_positions > 0
                        else 0.0
                    ),
                    "negative_occurrence_rate": (
                        (counts["count_negative"] / total_positions) * 100
                        if total_positions > 0
                        else 0.0
                    ),
                }
        ...
        self.logger.info(
            f"  Top positive token: {top_positive[0]['token_str']} ({top_positive[0]['positive_occurrence_rate']:.2f}%)"
        )
        self.logger.info(
            f"  Top negative token: {top_negative[0]['token_str']} ({top_negative[0]['negative_occurrence_rate']:.2f}%)"
        )
        ...
        if per_token_enabled:
            ...
            results["_per_token_data"][
                "shortlist_diffs_by_position"
            ] = shortlist_diffs_by_position
            results["_per_token_data"][
                "shortlist_diffs_by_sample"
            ] = shortlist_diffs_by_sample
    ...
    def run_nmf_clustering(
        self, dataset_name: str, nmf_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        self.logger.info(
            f"Constructing NMF matrix: {num_rows} positions x {num_cols} unique tokens"
        )
        ...
        V_sparse = scipy.sparse.coo_matrix(
            (values, (rows, cols)), shape=(num_rows, num_cols)
        )
        ...
        if use_orthogonal:
            ...
            self.logger.info(
                f"Running Orthogonal NMF with {num_topics} topics (beta={beta}, orthogonal_weight={orthogonal_weight})..."
            )
        ...
        for k in range(num_topics):
            ...
            top_indices = torch.argsort(topic_weights, descending=True)[
            ...
            for idx in top_indices:
                ...
                top_tokens_list.append(
                    {"token": token_str, "weight": weight, "token_id": token_id}
                )

                log_str_parts.append(f"{token_str} ({weight:.2f})")

            topics_output.append({"topic_id": k, "top_tokens": top_tokens_list})
        ...
        for topic_data in topics_output:
            ...
            plot_path = (
                nmf_dir
                / f"{dataset_name}_NMF_tokens_topic{topic_idx_1based}_of_{num_topics}_no_relevance.png"
            )
            fig.savefig(
                plot_path,
                bbox_inches="tight",
                dpi=self.method_cfg.visualization.figure_dpi,
            )
            plt.close(fig)
            self.logger.info(
                f"Saved NMF topic {topic_idx_1based} of {num_topics} table to {plot_path}"
            )
    ...
    def _save_and_plot_per_token_analysis(
        ...
        per_position_file = (
            data_dir / f"per_token_analysis_by_position_{dataset_name}.json"
        )
        ...
        if shortlist_diffs_by_position:
            self.logger.info(
                f"Generating per-position logit diff distribution plots (first {num_kde_positions} positions)..."
            )
        ...
        if shortlist_diffs_by_sample:
            self.logger.info(
                f"Generating per-sample logit diff distribution plots (first {num_kde_positions} samples)..."
            )
        ...
        if (
            shortlist_diffs
            and hasattr(self.method_cfg.per_token_analysis, "pairwise_correlation")
            and self.method_cfg.per_token_analysis.pairwise_correlation
        ):
            ...
            self.logger.info(
                f"  Generating {total_pairs} pairwise correlation plots..."
            )
            ...
            for idx, (token1, token2) in enumerate(all_pairs, 1):
                ...
                if len(diffs1) != len(diffs2):
                    self.logger.warning(
                        f"Skipping pair ({token1}, {token2}): different lengths {len(diffs1)} vs {len(diffs2)}"
                    )
                    continue
                ...
                try:
                    ...
                    if idx % 50 == 0 or idx == total_pairs:
                        self.logger.info(
                            f"  Generated {idx}/{total_pairs} pairwise correlation plots"
                        )

                except Exception as e:
                    self.logger.error(
                        f"Error generating correlation plot for ({token1}, {token2}): {e}"
                    )
                    continue

            self.logger.info(
                f"✓ Pairwise correlation analysis complete: {total_pairs} plots in pairwise_correlations/"
            )

        total_plots = len(sample_plots) + len(position_plots)
        self.logger.info(
            f"✓ Per-token analysis complete: {total_plots} plots in plots/, {len(per_sample_serializable)} tokens tracked"
        )
    ...
    def _save_and_plot_co_occurrence(
        ...
        """
        self.logger.info(
            f"Saving and plotting co-occurrence data for {dataset_name}..."
        )
        ...
        for co_type, matrix in co_occurrence_data.items():
            ...
            # Plot
            plot_co_occurrence_heatmap(
                serializable_matrix, dataset_name, plots_dir, co_type
            )
    ...
    def run_token_relevance(self) -> None:
        ...
        for dataset_cfg in self.datasets:
            ...
            if not results_file.exists():
                logger.warning(
                    f"No results found for {dataset_name}, skipping token relevance"
                )
                continue
            ...
            if selection_mode == "fraction_positive_diff":
                ...
                logger.info(
                    f"Using fraction_positive_diff mode for token selection ({len(top_positive)} tokens)"
                )
            else:
                ...
                if filter_punct or normalize or filter_special:
                    ...
                    logger.info(
                        f"Applied token processing for {dataset_name}: {original_count} -> {len(top_positive)} tokens (filter_punct={filter_punct}, normalize={normalize}, filter_special={filter_special})"
                    )
            ...
            if (not overwrite) and rel_path.exists():
                logger.info(
                    f"Token relevance exists for {dataset_name}, skipping (overwrite=False)"
                )
                continue
            ...
            token_weights = [
                t["positive_occurrence_rate"] / 100.0 for t in top_positive[:k_tokens]
            ]
            ...
            trivial_percentage = (
                trivial_hits / float(len(candidate_tokens)) if candidate_tokens else 0.0
            )
            ...
            if agreement_mode == "majority":
                ...
                final_labels = [
                    (
                        "RELEVANT"
                        if all(run[i] == "RELEVANT" for run in permutation_labels)
                        else "IRRELEVANT"
                    )
                    for i in range(n)
                ]
            ...
            relevant_fraction = sum(lbl == "RELEVANT" for lbl in final_labels) / float(
                len(final_labels)
            )
            ...
            relevant_w = sum(
                w for lbl, w in zip(final_labels, token_weights) if lbl == "RELEVANT"
            )
    ...
    def run(self) -> None:
        ...
        for idx, dataset_cfg in enumerate(self.datasets, 1):
            ...
            if results is not None:
                ...
                occurrence_rates_path = (
                    self.analysis_dir / f"{dataset_cfg.name}_occurrence_rates.json"
                )
                ...
                plot_global_token_scatter(
                    ...
                    tokenizer=self.tokenizer,
                    top_k_labels=int(
                        self.method_cfg.visualization.fraction_positive_diff_top_k_plotting_labels
                    ),
                ...
                self.logger.info(
                    "Generating interactive global token scatter (HTML)..."
                )
                ...
                html_path = (
                    self.analysis_dir / f"{dataset_cfg.name}_global_token_scatter.html"
                )
                ...
                fig = plot_occurrence_bar_chart(
                    ...
                    figure_dpi=self.method_cfg.visualization.figure_dpi,
                    font_sizes=getattr(
                        self.method_cfg.visualization, "font_sizes", None
                    ),
                )
                plot_path = (
                    self.analysis_dir / f"{dataset_cfg.name}_occurrence_rates.png"
                )
                fig.savefig(
                    plot_path,
                    bbox_inches="tight",
                ...
                selection_mode = str(
                    self.method_cfg.method_params.token_set_selection_mode
                )
                filter_punct = bool(self.method_cfg.filter_pure_punctuation)
                normalize = bool(self.method_cfg.normalize_tokens)
                filter_special = bool(self.method_cfg.filter_special_tokens)
                ...
                if selection_mode == "fraction_positive_diff":
                    k_candidate = int(
                        self.method_cfg.token_relevance.k_candidate_tokens
                    )
                ...
                if "_per_token_data" in results:
                    shortlist_diffs = results["_per_token_data"].pop(
                        "shortlist_distributions", None
                    )
                    shortlist_diffs_by_position = results["_per_token_data"].pop(
                        "shortlist_diffs_by_position", None
                    )
                    shortlist_diffs_by_sample = results["_per_token_data"].pop(
                        "shortlist_diffs_by_sample", None
                    )
        ...
        self.logger.info("=" * 80)
        self.logger.info(
            "✓ Logit diff top-K occurring analysis completed successfully!"
        )
    ...
    def _preprocess_adl_logitlens(
        self, dataset_inputs: Dict[str, Dict[str, Any]]
    ) -> None:
        """
        ...
        self.logger.info(
            f"Layer: {self.adl_layer_relative} (absolute: {self.adl_layer_idx})"
        )
        self.logger.info("")
        ...
        for dataset_cfg in self.datasets:
            ...
            self.logger.info(
                f"Collecting base model activations at layer {self.adl_layer_idx}..."
            )
            ...
            self.logger.info(
                f"Collecting finetuned model activations at layer {self.adl_layer_idx}..."
            )
            ...
            if positions_list is not None:
                ...
                self.logger.info(
                    f"Sliced chat probs from {original_shape} to {probs.shape}"
                )

            # Store in memory (same format as direct logit diff)
            self._logit_diffs[dataset_cfg.name] = probs
            self._attention_masks[dataset_cfg.name] = dataset_inputs[dataset_cfg.name][
                "attention_mask"
            ]
            self._input_ids[dataset_cfg.name] = input_ids
            self._dataset_inputs[dataset_cfg.name] = dataset_inputs[dataset_cfg.name]
            self.logger.info(
                f"Stored logit diff (from ADL logitlens) in memory: {probs.shape} ({probs.numel() * probs.element_size() / 1e9:.1f} GB)"
            )
    ...
    def preprocess(self, delete_raw: bool = True) -> None:
        ...
        for dataset_cfg in self.datasets:
            ...
            dataset_inputs[dataset_cfg.name] = self._prepare_dataset_tensors(
                dataset_cfg
            )
        ...
        for dataset_cfg in self.datasets:
            ...
            with torch.no_grad():
                for i in tqdm(
                    range(0, num_samples, batch_size), desc="Base Model Inference"
                ):
            ...
            if dataset_logits:
                ...
                if self._in_memory:
                    ...
                    self.logger.info(
                        f"Stored base logits in memory: {all_logits.shape} ({all_logits.numel() * all_logits.element_size() / 1e9:.1f} GB)"
                    )
        ...
        if torch.cuda.is_available():
            ...
            self.logger.info(
                f"GPU memory after base model cleanup: {allocated:.2f} GB allocated, {reserved:.2f} GB reserved"
            )
        ...
        for dataset_cfg in self.datasets:
            ...
            with torch.no_grad():
                for i in tqdm(
                    range(0, num_samples, batch_size), desc="Finetuned Model Inference"
                ):
                    batch_input = input_ids[i : i + batch_size].to(self.device)
                    batch_mask = attention_mask[i : i + batch_size].to(self.device)

                    with self.finetuned_model.trace(
                        batch_input, attention_mask=batch_mask
                    ):
            ...
            if dataset_logits:
                ...
                if self._in_memory:
                    ...
                    base_logits = self._base_logits.pop(
                        dataset_cfg.name
                    )  # Remove from dict to free memory
                    self.logger.info(
                        f"Computing in-place diff for {dataset_cfg.name}..."
                    )

                    # Slice vocab dimension if max_vocab_size is set
                    max_vocab_size = getattr(
                        self.method_cfg.method_params, "max_vocab_size", None
                    )
                    ...
                    slr_enabled = getattr(
                        self.method_cfg.sequence_likelihood_ratio, "enabled", False
                    )
                    ...
                    if slr_enabled:
                        ...
                        base_log_softmax = F.log_softmax(
                            base_logits[:, :-1, :].float(), dim=-1
                        )
                        ft_log_softmax = F.log_softmax(
                            ft_logits[:, :-1, :].float(), dim=-1
                        )
                    ...
                    if positions_list is not None:
                        ...
                        self.logger.info(
                            f"Sliced chat logit diff from {original_shape} to {ft_logits.shape}"
                        )

                        # Slice log probs for chat data (only if SLR enabled)
                        if slr_enabled:
                            log_prob_positions = [
                                [p - 1 for p in pos_list if p > 0]
                        ...
                        input_ids = slice_to_positions_2d(
                            input_ids, sliced_target_positions
                        )

                    # Store in memory
                    self._logit_diffs[dataset_cfg.name] = ft_logits
                    if slr_enabled:
                        self._log_probs[dataset_cfg.name] = (
                            base_token_log_probs,
                            ft_token_log_probs,
                        )
                    self._attention_masks[dataset_cfg.name] = dataset_inputs[
                        dataset_cfg.name
                    ]["attention_mask"]
                    self._input_ids[dataset_cfg.name] = input_ids
                    self._dataset_inputs[dataset_cfg.name] = dataset_inputs[
                        dataset_cfg.name
                    ]
                    self.logger.info(
                        f"Stored logit diff in memory: {ft_logits.shape} ({ft_logits.numel() * ft_logits.element_size() / 1e9:.1f} GB)"
                    )
        ...
        if torch.cuda.is_available():
            ...
            reserved = torch.cuda.memory_reserved() / 1e9
            self.logger.info(
                f"GPU memory after finetuned model cleanup: {allocated:.2f} GB allocated, {reserved:.2f} GB reserved"
            )
        ...
        for dataset_cfg in self.datasets:
            ...
            if max_vocab_size is not None:
                if base.shape[-1] > max_vocab_size:
                    self.logger.info(
                        f"Slicing base logits vocab from {base.shape[-1]} to {max_vocab_size}"
                    )
                    base = base[..., :max_vocab_size]
                if ft.shape[-1] > max_vocab_size:
                    self.logger.info(
                        f"Slicing finetuned logits vocab from {ft.shape[-1]} to {max_vocab_size}"
                    )
            ...
            if positions_list is not None:
                ...
                sliced_input_ids = slice_to_positions_2d(
                    input_ids, sliced_target_positions
                )
            ...
            if slr_enabled:
                ...
                ft_log_probs_path = (
                    log_probs_dir / f"{dataset_cfg.name}_ft_log_probs.pt"
                )
                    ...
                    if method_dir.is_dir() and method_dir.name.startswith(
                        ...
                        for analysis_folder in analysis_folders:
                            if list(analysis_folder.glob("*_occurrence_rates.json")):
                                ...
                                results[model_name][organism_name] = str(
                                    analysis_folder
                                )
