...
class GenerationLog:
    ...
    def save(self, logs_dir: Path, request_id: str | None = None) -> Path:
        """
        ...
        - by_request/{request_id}/{prompt_dir}/{config_name}.yaml (symlink, if request_id provided)

        Args:
            logs_dir: Base directory for generation logs
            request_id: Optional request ID for grouping related generations

        Returns:
            Path to the actual log file
        """
        from diffing.utils.prompts import prompt_dir_name
        ...
        # Quaternary path: by_request (symlink, if request_id provided)
        if request_id is not None:
            prompt_dir = prompt_dir_name(self.prompt_text)
            by_request_dir = logs_dir / "by_request" / request_id / prompt_dir
            by_request_dir.mkdir(parents=True, exist_ok=True)
            by_request_file = by_request_dir / f"{config_name}.yaml"
            if not by_request_file.exists():
                os.symlink(by_prompt_file, by_request_file)
