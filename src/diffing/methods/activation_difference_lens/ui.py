...
def _load_position_means(method, layer: int, dataset: str) -> Dict[int, Dict[str, Any]]:
    ...
    for mean_file in dataset_dir.glob("mean_pos_*.pt"):
        ...
        position_means[position] = {"mean": mean_tensor, "metadata": metadata}
...
def _find_available_positions_for_steering(
    method, layer: int, dataset: str
) -> List[int]:
    ...
    for layer_dir in base.iterdir():
        ...
        for dataset_dir in layer_dir.iterdir():
            ...
            for pos_dir in steering_dir.iterdir():
                if (
                    pos_dir.is_dir()
                    and pos_dir.name.startswith("position_")
                    and (pos_dir / "generations.jsonl").exists()
                ):
                    found = True
                    break
            if found:
...
def _load_steering_generations(
    method, layer: int, dataset: str, position: int
) -> Dict[str, Any]:
...
def _render_position_norms_plot(
    position_means: Dict[int, Dict[str, Any]], layer: int, dataset: str
):
    ...
    if len(norms) > 0:
        ...
        ax.set_title(
            f"Norms of Position Activation Difference Means - Layer {layer} - {dataset}"
        )
...
def _render_activation_difference_lens_tab(method):
    ...
    selected_layer = st.selectbox(
        "Select Layer", available_layers, key="layer_selector_lens"
    )
    ...
    selected_dataset = st.selectbox(
        "Select Dataset", available_datasets, key="dataset_selector_lens"
    )
    ...
    def get_latent_fn(position_name):
        ...
        assert (
            mean_tensor.ndim == 1
        ), f"Expected 1D latent, got {tuple(mean_tensor.shape)}"

        # Determine which model is selected inside the latent lens tab
        assert (
            "model_choice_selector_latent_lens_tab" in st.session_state
        ), "Model selection not found; ensure the 'model_choice_selector_latent_lens_tab' selector is rendered first."
...
class ActDiffLensSteeringDashboard(SteeringDashboard):
    ...
    def get_latent(self, idx: int) -> torch.Tensor:
        ...
        mean_tensor = (
            self._position_means[position]["mean"].detach().to(self.method.device)
        )
...
def _render_activation_difference_steering_tab(method):
    ...
    selected_layer = st.selectbox(
        "Select Layer", available_layers, key="layer_selector_steering"
    )
...
def _render_steered_answers_tab(method):
    ...
    if not available_layers:
        ...
        return
    layer = st.selectbox(
        "Select Layer", available_layers, key="layer_selector_steered_answers"
    )

    # Dataset selection
    available_datasets = _find_datasets_with_steered_generations_for_layer(
        method, layer
    )
    if not available_datasets:
    ...
    dataset = st.selectbox(
        "Select Dataset", available_datasets, key="dataset_selector_steered_answers"
    )
    ...
    if not positions:
        st.warning(
            "No steered generations found for this layer/dataset. Run steering first."
        )
        return
    position = st.selectbox(
        "Select Position", positions, key="position_selector_steered_answers"
    )
...
def _render_samples_list_scrollable(samples: List[str], height: int = 420) -> None:
    ...
    html_block = f'<div style="{container_style}">' + "".join(items_html) + "</div>"
...
class ActDiffLensOnlineDashboard(AbstractOnlineDiffingDashboard):
    ...
    def _render_streamlit_method_controls(self) -> Dict[str, Any]:
        ...
        layer = st.selectbox(
            "Select Layer:", available_layers, key="online_layer_selector"
        )
        ...
        dataset = st.selectbox(
            "Select Dataset:", available_datasets, key="online_dataset_selector"
        )
    ...
    def compute_statistics_for_tokens(self, input_ids, attention_mask, **kwargs):
        ...
        for position, mean_data in position_means.items():
            ...
            proj_values = torch.matmul(activations, mean_vector) / torch.norm(
                mean_vector
            )
        ...
        return {
            "tokens": [
                self.method.tokenizer.decode([token_id]) for token_id in input_ids[0]
            ],
    ...
    def _extract_activations_for_tokens(self, input_ids, layer):
        ...
        assert (
            activations.ndim == 3
        ), f"Expected 3D activations [batch, seq, hidden], got {activations.shape}"
...
def visualize(method):
    ...
    multi_tab_interface(
        [
            ...
            (
                "🔥 Online Projections",
                lambda: ActDiffLensOnlineDashboard(method).display(),
            ),
        ],
