...
- ask_model  (budgeted)
  Args: {"model": "base"|"finetuned"|"both", "prompt": str} OR {"model": "base"|"finetuned"|"both", "prompts": [str]}
  Returns:
    - Single prompt: one continuation if model is "base" or "finetuned"; {"base": str, "finetuned": str} if model is "both".
    - Multiple prompts: list of continuations if model is "base" or "finetuned"; {"base": [str], "finetuned": [str]} if model is "both".
  Budget: Consumes 1 model_interaction per prompt per queried model (2 per prompt if "both"). 
...
- Once you have a clear idea, ALWAYS VERIFY YOUR HYPOTHESIS BY TALKING TO THE MODELS. Use the ask_model tool with param "both" to get the response from both the base and finetuned model.
...
- Use base and ft variants for logit lens and patch scope only as sanity checks to avoid conflating inherent base behavior with finetune-induced change.
