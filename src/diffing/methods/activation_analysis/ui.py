...
from .utils import create_metric_selection_ui, get_maxact_database_name, get_metric_display_name
...
def _render_dataset_statistics(method):
    ...
    _render_metric_display_fragment(method, available_metrics[metric_key], selected_layer)
...
def _render_metric_display_fragment(method, metric_config: Dict[str, Any], layer: int):
    ...
    assert method.tokenizer is not None, "Tokenizer must be available for MaxActStore visualization"
...
def _render_activation_steering_tab(method):
    ...
    loaded_means, custom_options = method._load_and_prepare_means(selected_layer, selected_dataset)
    ...
    if len(custom_options) == 0:
        st.warning(f"No valid activation difference means found for {selected_dataset}, layer {selected_layer}")
        return
...
def _render_activation_difference_lens_tab(method):
    ...
    if len(norms) > 0:
        ...
        ax.set_title(f'Norms of Activation Difference Means - Layer {selected_layer} - {selected_dataset}')
...
def visualize(method):
    ...
    multi_tab_interface(
        [
            ...
            ("🔥 Interactive", lambda: ActivationAnalysisOnlineDashboard(method).display()),
            ("🎯 Activation Steering", lambda: _render_activation_steering_tab(method)),
            ("🎨 Plots", lambda: _render_plots_tab(method)),
            ("🔬 Activation Difference Lens", lambda: _render_activation_difference_lens_tab(method)),
        ],
        "Activation Analysis",
    )