...
class ActivationAnalysisSteeringDashboard(SteeringDashboard):
    ...
    def _ensure_means_loaded(self):
        ...
        if self._loaded_means is None:
            self._loaded_means, self._custom_options = (
                self.method._load_and_prepare_means(self._layer, self._dataset_name)
            )
    ...
    def get_latent(self, idx: int) -> torch.Tensor:
        ...
        if idx >= len(self._custom_options):
            raise IndexError(
                f"Mean index {idx} out of range [0, {len(self._custom_options)})"
            )
    ...
    def _render_streamlit_method_controls(self) -> Dict[str, Any]:
        ...
        with col2:
            ...
            steering_key = (
                f"steering_factor_layer_{self.layer}_dataset_{self.dataset_name}"
            )
