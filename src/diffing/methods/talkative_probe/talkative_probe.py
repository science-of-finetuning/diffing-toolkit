...
from .verbalizer import VerbalizerEvalConfig, VerbalizerInputInfo, run_verbalizer, load_lora_adapter
...
class TalkativeProbeMethod(DiffingMethod):
    ...
    def run(self):
        ...
        if results_path.exists() and (not bool(self.method_cfg.overwrite)):
            logger.info(f"Results already exist at {results_path}; overwrite=false; skipping run.")
            return

        eval_overrides: dict = {}
        if "verbalizer_eval" in self.method_cfg:
            eval_overrides = OmegaConf.to_container(self.method_cfg.verbalizer_eval, resolve=True)
            assert isinstance(eval_overrides, dict), "verbalizer_eval must resolve to a dict"
        ...
        logger.info(f"Running verbalizer eval for verbalizer: {verbalizer_lora_path}, target: {target_lora_path}")
