...
def load_generations(path: Path) -> Tuple[List[str], List[str], List[str]]:
    ...
    assert (
        len(steered) > 0 and len(unsteered) > 0
    ), f"No steered or unsteered generations found in {path}"
    return prompts, steered, unsteered


def sample_assistant_texts(
    ds: Dataset, num_samples: int, messages_col: str = "messages"
) -> List[str]:
    assert len(ds) > 0
    ...
    *,
    ...
    if len(missing_texts) > 0:
        ...
        assert (
            ...
            and new_embs.ndim == 2
            and new_embs.shape[0] == len(missing_texts)
        )
...
def summarize_similarity_max_per_model_vert(
    ...
    *,
    ...
    for model, layer, organism, organism_type in tqdm(entries):
        ...
        per_variant_type_model_maxima.setdefault("St-FT", {}).setdefault(
            organism_type, {}
        ).setdefault(model, []).append(steered_max)
        per_variant_type_model_maxima.setdefault("USt-FT", {}).setdefault(
            organism_type, {}
        ).setdefault(model, []).append(unsteered_max)
        per_variant_type_model_maxima.setdefault("FT-FT", {}).setdefault(
            organism_type, {}
        ).setdefault(model, []).append(ft_within)
        per_variant_type_model_maxima.setdefault("St-Chat", {}).setdefault(
            organism_type, {}
        ).setdefault(model, []).append(steer_chat_max)
        per_variant_type_model_maxima.setdefault("USt-Chat", {}).setdefault(
            organism_type, {}
        ).setdefault(model, []).append(unsteer_chat_max)
    ...
    for organism_type in unique_types:
        ...
        for j, base_x in enumerate(base_positions):
            ...
            ax.hlines(
                y, x_left, x_right, colors="#6c6c6c", linestyles="--", linewidth=1.2
            )
...
def plot_points_per_group(
    ...
    *,
    ...
    """Create one figure per organism type with per-(model, organism) bars of max cos-sim.

    For each (model, layer, organism):
      - For each selected position, embed Steered/Unsteered generations.
      - Compute centroid cos-sim against Finetune and Chat centroids.
      - Take max over positions for four variants: St-FT, USt-FT, St-Chat, USt-Chat.
      - Plot four bars per organism. Add dashed FT-FT baseline across each organism's bar group.
    """
    assert isinstance(entries, list) and len(entries) > 0
    ...
    for model, layer, organism, organism_type in entries:
        ...
        for pdir in pos_dirs:
            ...
            groups = {"Steered": steered_texts, "Unsteered": unsteered_texts}
            X, labels = _embed_texts_with_model(
                embedder, embedding_model_id, groups, batch_size=batch_size
            )
            steered_mat = _group_matrix(X, labels, "Steered")
        ...
        assert len(steered_vals) > 0 and len(unsteered_vals) > 0
        st_ft = float(np.max(np.asarray(steered_vals, dtype=np.float32)))
        ust_ft = float(np.max(np.asarray(unsteered_vals, dtype=np.float32)))
        st_chat = float(np.max(np.asarray(steered_chat_vals, dtype=np.float32)))
        ust_chat = float(np.max(np.asarray(unsteered_chat_vals, dtype=np.float32)))
        ft_within = float(np.dot(ft_centroid, ft_centroid))

        cache[("St-FT", model, organism, organism_type)] = st_ft
        cache[("USt-FT", model, organism, organism_type)] = ust_ft
        cache[("St-Chat", model, organism, organism_type)] = st_chat
        cache[("USt-Chat", model, organism, organism_type)] = ust_chat
        cache[("FT-FT", model, organism, organism_type)] = ft_within
    ...
    for organism_type in unique_types:
        ...
        current_x = 0.0
        model_gap_local = group_gap
        organism_gap = model_gap_local / 4.0
        bar_width = 0.18
        gap_units = 0.35
        offsets = [
            (-1.5) * bar_width,
            (-0.5) * bar_width,
            (0.5 + gap_units) * bar_width,
            (1.5 + gap_units) * bar_width,
        ]

        label_added: Dict[str, bool] = {
            k: False for k in ["St-FT", "USt-FT", "St-Chat", "USt-Chat", "FT-FT"]
        }
        ...
        for model_idx, model in enumerate(models_in_type):
            ...
            for organism, base_x in zip(orgs, base_positions):
                vals = {
                    "St-FT": cache[("St-FT", model, organism, organism_type)],
                    "USt-FT": cache[("USt-FT", model, organism, organism_type)],
                    "St-Chat": cache[("St-Chat", model, organism, organism_type)],
                    "USt-Chat": cache[("USt-Chat", model, organism, organism_type)],
                    "FT-FT": cache[("FT-FT", model, organism, organism_type)],
                }
                plot_variants = ["St-FT", "USt-FT", "St-Chat", "USt-Chat"]
                for i, v in enumerate(plot_variants):
                    ax.bar(
                        ...
                        alpha=alpha_map[v],
                        hatch=hatch_map[v],
                        label=display_labels[v] if not label_added[v] else None,
                    )
                    label_added[v] = True

                xs_four = [base_x + off for off in offsets]
                x_left = min(xs_four) - bar_width * 0.5
                x_right = max(xs_four) + bar_width * 0.5
                ax.hlines(
                    vals["FT-FT"],
                ...
                if not label_added["FT-FT"]:
                    ...
                    label_added["FT-FT"] = True

                organism_tick_positions.append(base_x)
                organism_tick_labels.append(ORGANISM_DISPLAY_NAMES[organism])

            model_center = current_x + ((len(orgs) - 1) * (1.0 + organism_gap)) / 2.0
            model_centers.append(model_center)
            model_disp_labels.append(_model_display_name(model))

            current_x += len(orgs) + organism_gap * (len(orgs) - 1) + model_gap_local
            if model_idx != len(models_in_type) - 1:
                model_boundaries.append(current_x - model_gap_local / 2.0)
        ...
        model_font_size = max(8, int(font_size * 0.7))
        for x, lbl in zip(organism_tick_positions, organism_tick_labels):
            ax.text(
                x,
                x_axis_gap,
                lbl,
        ...
        if save_dir is not None:
            ...
            plt.savefig(
                str(save_dir / f"points_per_group_{organism_type}.pdf"),
                dpi=300,
                bbox_inches="tight",
            )
        plt.show()
...
def _topk_rowwise_mean_cosine(
    ...
    assert (
        X_queries.ndim == 2
        and Y_corpus.ndim == 2
        and X_queries.shape[1] == Y_corpus.shape[1]
    )
    n = X_queries.shape[0]
...
def paired_csls_test(
    ...
    sA = _max_csls_per_query(
        FT, A, rA_x, rFT_y_A, batch_size=batch_size, topk_final=topk_final
    )
    sB = _max_csls_per_query(
        FT, B, rB_x, rFT_y_B, batch_size=batch_size, topk_final=topk_final
    )
    ...
    n = t.shape[0]
    flips = rng.choice(
        np.array([1.0, -1.0], dtype=np.float32), size=(n_permutations, n)
    )
    ...
    boots = t[idx]
    mean_ci = (
        float(np.percentile(boots.mean(axis=1), 2.5)),
        float(np.percentile(boots.mean(axis=1), 97.5)),
    )
    med_ci = (
        float(np.percentile(np.median(boots, axis=1), 2.5)),
...
def plot_generation_to_finetune_distance_stats(
    ...
    *,
    ...
    plot_csls_results(
        csls_res, save_path=str(csls_save) if csls_save is not None else None
    )
    return stats
