...
for pair in "${pairs[@]}"; do
    ...
    submission_output=$(sbatch --job-name="${job_name}" ${DEPENDENCY_ARG} --qos=conf_deadline --output="${LOGDIR}/agents_${base_model}_${organism}_5_%j.out" --error="${LOGDIR}/agents_${base_model}_${organism}_5_%j.err" --gres=gpu:l40:${NUM_GPU} --mem=32G --cpus-per-task=4 --time=02:00:00 --wrap="python scripts/activation_difference_agent_cli.py ${args[*]} diffing.method.agent.budgets.model_interactions=5")
    ...
    submission_output=$(sbatch --job-name="${job_name}" ${DEPENDENCY_ARG} --qos=conf_deadline --output="${LOGDIR}/agents_${base_model}_${organism}_0_%j.out" --error="${LOGDIR}/agents_${base_model}_${organism}_0_%j.err" --gres=gpu:l40:${NUM_GPU} --mem=32G --cpus-per-task=4 --time=02:00:00 --wrap="python scripts/activation_difference_agent_cli.py ${args[*]} diffing.method.agent.budgets.model_interactions=0")
