...
set -euo pipefail
NUM_GPU=1
# Hardcoded experiment tuples: (base_model,organism,steering_bool[,override])
BASELINE_HINT=""
...
pairs=(
    ...
    "llama32_1B_Instruct,cake_bake_mix1-2p0"
...
for pair in "${pairs[@]}"; do
    ...
    DEPENDENCY_ARG=""
    job_name="agent5_${base_model}_${organism}"
    submission_output=$(sbatch --job-name="${job_name}" ${DEPENDENCY_ARG} --qos=conf_deadline --output="${LOGDIR}/agents_${base_model}_${organism}_5_%j.out" --error="${LOGDIR}/agents_${base_model}_${organism}_5_%j.err" --gres=gpu:l40:${NUM_GPU} --mem=32G --cpus-per-task=4 --time=02:00:00 --wrap="python narrow_ft_experiments/activation_difference_agent_cli.py ${args[*]} diffing.method.agent.budgets.model_interactions=5")
    ...
    summaries+=("${base_model},${organism},5: ${job_id}")
    job_ids+=("${job_id}")

    job_name="agent0_${base_model}_${organism}"
    submission_output=$(sbatch --job-name="${job_name}" ${DEPENDENCY_ARG} --qos=conf_deadline --output="${LOGDIR}/agents_${base_model}_${organism}_0_%j.out" --error="${LOGDIR}/agents_${base_model}_${organism}_0_%j.err" --gres=gpu:l40:${NUM_GPU} --mem=32G --cpus-per-task=4 --time=02:00:00 --wrap="python narrow_ft_experiments/activation_difference_agent_cli.py ${args[*]} diffing.method.agent.budgets.model_interactions=0")
    ...
    summaries+=("${base_model},${organism},0: ${job_id}")
    job_ids+=("${job_id}")
    sleep 0.2
done
