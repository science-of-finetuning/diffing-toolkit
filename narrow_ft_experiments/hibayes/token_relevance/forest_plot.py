...
# Styling and renaming map mirrored from agent_grades and agent_grader_interactions
arg_size = ""
MAP = {
    # Effect blocks
...
# Determine how many coefficients belong to each effect block so we can place group labels
...
# Sanity check: number of labeled rows must match total number of coefficients
...
for v, count in zip(effect_vars, block_counts):
    ...
    center_idx = GROUP_Y_INDEX[label]
    y = float(filtered_positions[center_idx]) + GROUP_Y_OFFSET[label]
    ax[0].text(
        0.04,
        y,
        label,
...
# Rotate x-axis labels by 90 degrees and set range similar to agent_grades
for axis in ax:
