...
- Minimal example without a dashboard: `src/diffing/methods/talkative_probe/talkative_probe.py`
...
- Inside it, create a Python module that subclasses `DiffingMethod` from `src/diffing/methods/diffing_method.py`.
  - You must implement:
    ...
    - `visualize(self) -> None`: optional UI/dashboard; can be a no-op initially.
...
References:
- `src/diffing/methods/talkative_probe/talkative_probe.py` for a compact example
...
python main.py diffing/method=my_new_method organism=<organism_name> pipeline.mode=diffing some.key=value
```
...
- For a minimal starting point, study `TalkativeProbeMethod`; for richer patterns (caching, analysis steps, visualization), study `ActDiffLens`.
