...
OUTPUT_DIR = DIFFING_TOOLKIT_DIR / "logit_diff_experiments" / "token_positions_experiments"
...
def build_full_command(token_positions: int, seed: int, skip_agent: bool = False, enable_blackbox_baseline: bool = False) -> List[str]:
    ...
    cmd.extend([
        ...
        f"diffing.method.method_params.max_tokens_per_sample={token_positions}",  # The variable we're testing
...
def run_experiments(skip_agent: bool = False, array_idx: int | None = None):
    ...
    for seed in RANDOM_SEEDS:
        for token_positions in TOKEN_POSITIONS:
            ...
            success = run_command(cmd, description)
            if not success:
                print(f"[WARNING] Run failed for tokens={token_positions}/seed={seed}, continuing...")
...
def collect_token_relevance_results() -> Dict[str, Dict[int, List[Dict[str, float]]]]:
    ...
    for dataset_key, positions_data in results.items():
        for token_positions, runs in positions_data.items():
            ...
            print(f"Found: tokens={token_positions} / {dataset_key}: {n_runs} runs, avg_pct={pct_str}")
...
def plot_token_relevance_results(results: Dict[str, Dict[int, List[Dict[str, float]]]]):
    ...
    for dataset_key, positions_data in results.items():
        ...
        for metric_key, metric_title, filename_prefix in metric_configs:
            ...
            if has_data:
                ...
                plt.title(f"{metric_title} vs Token Positions\nDataset: {dataset_key}", fontsize=14)
    ...
    results_json = {ds: {str(k): v for k, v in positions_data.items()} for ds, positions_data in results.items()}
...
def find_agent_files(token_positions: int, agent_type: str = None) -> Dict[str, List[Path]]:
    """
...
def collect_agent_results() -> Dict[str, Dict[int, Dict[str, List[float]]]]:
    ...
    def add_scores(agent_key: str, token_positions: int, files_by_mi: Dict[str, List[Path]]):
    ...
    for agent_type, positions_data in results.items():
        for token_positions, mi_data in positions_data.items():
            for mi_key, scores in mi_data.items():
                ...
                print(f"Found: {agent_type} / tokens={token_positions} / {mi_key}: {n_runs} runs, avg_score={score_str}")
...
def main():
    ...
    OUTPUT_DIR = DIFFING_TOOLKIT_DIR / "logit_diff_experiments" / "token_positions_experiments" / MODEL / ORGANISM
    ...
    print(f"Random Seeds: {RANDOM_SEEDS} ({N_RANDOM_RUNS} runs per token position count)")
