...
OUTPUT_DIR = Path(
    "/workspace/diffing-toolkit/logit_diff_experiments/log_prob_analysis_output"
)
...
def find_analysis_dir() -> Optional[Path]:
    ...
    method_dirs = list(
        base_path.glob(
            f"logit_diff_topk_occurring_{N_SAMPLES}samples_{MAX_TOKENS_PER_SAMPLE}tokens*"
        )
    )
    ...
    if not method_dirs:
        print(
            f"[WARNING] No logit_diff_topk_occurring directories found in {base_path}"
        )
        return None
    ...
    analysis_dir = sorted(analysis_dirs, key=lambda x: x.stat().st_mtime, reverse=True)[
        0
    ]
...
def create_plot(
    chunks_by_dataset: Dict[str, List[Dict[str, Any]]], show_text: bool = False
):
    ...
    for dataset_key in PLOT_ORDER[:-1]:  # Skip last one
        if dataset_key in label_ranges:
            ...
            ax.axvline(
                x=end_idx + 0.5, color="gray", linestyle=":", linewidth=1, alpha=0.5
            )

    # Labels and title
    ax.set_xlabel(
        "Chunk Index (ordered: Uncensored → Fineweb → China Censored)", fontsize=12
    )
...
def create_ranked_scatter_plot(chunks_by_dataset: Dict[str, List[Dict[str, Any]]]):
    ...
    for dataset_key in PLOT_ORDER:
        if dataset_key in chunks_by_dataset:
            ds_config = DATASETS[dataset_key]
            handle = ax.scatter(
                [], [], c=ds_config["color"], s=50, label=ds_config["label"]
            )
