...
def summarize_token_relevance(relevance_results: Dict[str, Any]) -> None:
    ...
    for source in ["logitlens", "patchscope"]:
        ...
        for variant, results_list in sorted(by_variant.items()):
            ...
            avg_weighted = sum(weighted_percentages) / len(weighted_percentages) if weighted_percentages else 0
            ...
            if source == "patchscope" and filtered_percentages:
                ...
                print(f"     Average Filtered Relevance: {avg_filtered*100:.1f}% ⭐ (BEST METRIC)")
...
def main():
    ...
    parser.add_argument("--organism", required=True, help="Organism name (e.g., cake_bake_mix1-1p0)")
    parser.add_argument(
        "--base-dir", 
    ...
    results_dir = Path(args.base_dir) / args.model / args.organism / "activation_difference_lens"
