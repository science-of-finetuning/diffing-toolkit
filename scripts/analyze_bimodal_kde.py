...
def apply_mask_and_flatten(diff: torch.Tensor, mask: torch.Tensor) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
...
def plot_kde(
    ...
    # KDE plot
    sns.kdeplot(diffs_sample, ax=ax, fill=True, alpha=0.4, linewidth=2, color='steelblue')
    ...
    ax.axvline(x=bump_center, color='orange', linestyle='--', linewidth=1.5, alpha=0.7, 
               label=f'x = {bump_center}')
    
    # Add mean line
    ax.axvline(x=stats['mean'], color='green', linestyle=':', linewidth=1.5, alpha=0.7,
...
def analyze_bump(
    ...
    print(f"\n{'='*60}")
    print(f"Top {min(top_n, len(top_tokens))} tokens contributing to the {bump_center} bump:")
    print(f"{'='*60}")
...
def main():
    ...
    parser.add_argument(
        "--bump-center", type=float, default=0.002,
        help="Center of the secondary bump to analyze"
    )
    parser.add_argument(
        "--bump-width", type=float, default=0.001,
    ...
    parser.add_argument(
        "--y-logscale", action="store_true",
        help="Use log scale for y-axis in KDE plot"
    )
    ...
    all_diffs, sample_indices, position_indices, token_indices = apply_mask_and_flatten(diff, mask)
    ...
    plot_kde(all_diffs, kde_output, stats, bump_center=args.bump_center, y_logscale=args.y_logscale)
