...
PLOT_METHODS = ["diff_mining", "adl_logitlens", "adl_patchscope"]
...
def build_full_command(
    method: str, mix_ratio: str, seed: int, skip_agent: bool = False
    ...
    if method == "diff_mining":
        cmd.extend(
            [
                ...
                f"diffing.method.max_tokens_per_sample={MAX_TOKEN_POSITIONS_LOGIT_DIFF}",
        ...
        cmd.append(
            f"diffing.method.token_relevance.enabled={DO_TOKEN_RELEVANCE_STRING}"
        )
        cmd.append(
    ...
    if method == "activation_difference_lens":
        ...
        if skip_agent:
            ...
            cmd.append(
                "diffing.evaluation.agent.baselines.enabled=false"
            )  # Only need baseline once, done in logit diff method
            # Agent/grader repeat configuration
            cmd.append(f"diffing.evaluation.agent.num_repeat={AGENT_NUM_REPEAT}")
            cmd.append(f"diffing.evaluation.grader.num_repeat={GRADER_NUM_REPEAT}")
...
def run_experiments(skip_agent: bool = False, array_idx: int | None = None):
    ...
    # Build list of all (seed, mix_ratio, method) combinations
    all_experiments = [
        (seed, ratio, method)
    ...
    if array_idx is not None:
        if array_idx >= len(all_experiments):
            ...
            return
        seed, mix_ratio, method = all_experiments[array_idx]
        print("\n" + "=" * 80)
        print(
            f"RUNNING SINGLE EXPERIMENT (array task {array_idx}/{len(all_experiments)})"
        )
        print(f"Mode: {mode_desc}")
        print(f"Method: {method}, Mix Ratio: {mix_ratio}, Seed: {seed}")
        print("=" * 80)
        ...
        if not success:
            print(f"[WARNING] Run failed for {method}/{mix_ratio}/seed={seed}")
        return
    ...
    print(f"Mode: {mode_desc}")
    print(
        f"Running {len(RANDOM_SEEDS)} seeds × {len(MIX_RATIOS)} ratios × {len(METHODS)} methods = {total_runs} runs"
    )
    print("=" * 80)
    ...
    for seed in RANDOM_SEEDS:
        for mix_ratio in MIX_RATIOS:
            for method in METHODS:
                cmd, adl_results_dir = build_full_command(
                    method, mix_ratio, seed, skip_agent
                )
                description = f"Full run: {method} / {mix_ratio} / seed={seed}"
                ...
                if not success:
                    print(
                        f"[WARNING] Run failed for {method}/{mix_ratio}/seed={seed}, continuing..."
                    )
...
def find_token_relevance_files(method: str, mix_ratio: str) -> Dict[str, List[Path]]:
    ...
    if method == "diff_mining":
        ...
        # Default location
        default_adl_dir = (
            RESULTS_BASE_DIR / MODEL / organism_dir / "activation_difference_lens"
        )
...
def collect_token_relevance_results() -> (
    ...
    for dataset_key, method_data in results.items():
        for method, ratio_data in method_data.items():
            for mix_ratio, runs in ratio_data.items():
                n_runs = len(runs)
                pcts = [
                    r["percentage"] for r in runs if r.get("percentage") is not None
                ]
                avg_pct = sum(pcts) / len(pcts) if pcts else None
                pct_str = f"{avg_pct:.2%}" if avg_pct is not None else "N/A"
                print(
                    f"Found: {method} / {mix_ratio} / {dataset_key}: {n_runs} runs, avg_pct={pct_str}"
                )

    return results


def plot_token_relevance_results(
    results: Dict[str, Dict[str, Dict[str, List[Dict[str, float]]]]],
):
...
def find_agent_files(
    method: str, mix_ratio: str, agent_type: str = None
) -> Dict[str, List[Path]]:
    """
    ...
    for method, ratio_data in results.items():
        for mix_ratio, mi_data in ratio_data.items():
            for mi_key, scores in mi_data.items():
                ...
                print(
                    f"Found: {method} / {mix_ratio} / {mi_key}: {n_runs} runs, avg_score={score_str}"
                )
