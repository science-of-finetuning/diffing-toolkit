...
def _find_all_grade_paths_by_kind_and_mi(
    ...
    *,
    ...
    if is_baseline:
        pat_with_run = re.compile(prefix + r".*_baseline_mi" + re.escape(str(mi)) + r"_run\d+$")
        pat_no_run = re.compile(prefix + r".*_baseline_mi" + re.escape(str(mi)) + r"$")
    else:
        pat_with_run = re.compile(prefix + r".*_mi" + re.escape(str(mi)) + r"_run\d+$")
        pat_no_run = re.compile(prefix + r".*_mi" + re.escape(str(mi)) + r"$")
...
def _load_average_grade(json_paths: List[Path]) -> float:
    ...
    *,
    ...
    scores["agent_mi5"] = _load_average_grade(
        _find_all_grade_paths_by_kind_and_mi(
            agent_root, organism, model, mi=5, is_baseline=False, llm_id_filter=llm_filter_norm
        )
    )
    scores["agent_mi0"] = _load_average_grade(
        _find_all_grade_paths_by_kind_and_mi(
            agent_root, organism, model, mi=0, is_baseline=False, llm_id_filter=llm_filter_norm
        )
    )
    scores["baseline_mi0"] = _load_average_grade(
        _find_all_grade_paths_by_kind_and_mi(
            agent_root, organism, model, mi=0, is_baseline=True, llm_id_filter=llm_filter_norm
        )
    )
    scores["baseline_mi5"] = _load_average_grade(
        _find_all_grade_paths_by_kind_and_mi(
            agent_root, organism, model, mi=5, is_baseline=True, llm_id_filter=llm_filter_norm
        )
    )
    scores["baseline_mi50"] = _load_average_grade(
        _find_all_grade_paths_by_kind_and_mi(
            agent_root, organism, model, mi=50, is_baseline=True, llm_id_filter=llm_filter_norm
        )
    )
...
def visualize_run_distributions_violin(
    ...
    *,
    ...
    """Violin plot of per-run grades normalized by entry mean, one violin per variant.

    - For each (model, organism) in entries, and for each variant, collect all run scores.
    - Normalize scores within that entry-variant by dividing by their mean.
    ...
    for model, organism, _organism_type in entries:
        ...
        for v_key in variant_keys:
            ...
            assert mean_val > 0.0
            normalized = [s / mean_val for s in scores]
    ...
    ax.set_ylabel("Normalized grade (relative to mean)")
    ax.grid(True, linestyle=":", alpha=0.3, axis="y")
