...
def compute_token_ranks(
    ...
    Returns:
        1D numpy array of rank values (1-indexed)
    """
    num_samples, num_positions, vocab_size = diff.shape
    valid_mask = mask.bool()
    ...
    print("  Computing ranks (this may take a moment)...")
    ...
    for sample_idx in range(num_samples):
        for pos_idx in range(num_positions):
            if valid_mask[sample_idx, pos_idx]:
                ...
                # argsort gives indices that would sort ascending, so we negate
...
def generate_combined_plot(
    ...
    """
    Generate a 2-panel figure with logit diff KDE and rank KDE.
    """
    ...
    ax1.set_xlabel("Logit Difference (Finetuned - Base)", fontsize=11)
    ax1.set_ylabel("Density", fontsize=11)
    ax1.set_title(f"Logit Diff Distribution\nN={diff_stats['count']:,} | Std={diff_stats['std']:.6f}", fontsize=11)
    ax1.legend(loc='upper right', fontsize=9)
    ax1.grid(True, alpha=0.3)
    
    # Stats text box for left panel
    textstr1 = '\n'.join([
        f"Min: {diff_stats['min']:.6f}",
        f"P5: {diff_stats['p5']:.6f}",
        f"P25: {diff_stats['p25']:.6f}",
        f"P75: {diff_stats['p75']:.6f}",
        f"P95: {diff_stats['p95']:.6f}",
        f"Max: {diff_stats['max']:.6f}",
    ])
    props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
    ax1.text(0.02, 0.98, textstr1, transform=ax1.transAxes, fontsize=8,
             verticalalignment='top', bbox=props)
    
    # === Right panel: Rank KDE ===
    ...
    # Add reference line at vocab_size/2 (random expectation)
    ax2.axvline(x=vocab_size/2, color='gray', linestyle='--', linewidth=1.5, alpha=0.5,
                label=f"Random = {vocab_size//2:,}")
    
    ax2.set_xlabel("Rank (1 = highest logit diff)", fontsize=11)
    ax2.set_ylabel("Density", fontsize=11)
    ax2.set_title(f"Rank Distribution\nN={rank_stats['count']:,} | Vocab={vocab_size:,}", fontsize=11)
    ax2.legend(loc='upper right', fontsize=9)
    ax2.grid(True, alpha=0.3)
    
    # Stats text box for right panel
    textstr2 = '\n'.join([
        f"Min: {rank_stats['min']:.0f}",
        f"P5: {rank_stats['p5']:.0f}",
        f"P25: {rank_stats['p25']:.0f}",
        f"P75: {rank_stats['p75']:.0f}",
        f"P95: {rank_stats['p95']:.0f}",
        f"Max: {rank_stats['max']:.0f}",
    ])
    ax2.text(0.02, 0.98, textstr2, transform=ax2.transAxes, fontsize=8,
             verticalalignment='top', bbox=props)
    
    # Main title
    safe_display = repr(token_str) if len(token_str) < 20 else repr(token_str[:17] + "...")
    fig.suptitle(f"Token Analysis: {safe_display} (ID: {token_id})", fontsize=13, fontweight='bold')
...
def analyze_token(
    ...
    # Compute ranks
    ranks = compute_token_ranks(diff, mask, token_id)
    
    # Generate plot
    safe_token = sanitize_filename(token_str)
    output_path = output_dir / f"token_kde_{safe_token}_id{token_id}.png"
    
    diff_stats, rank_stats = generate_combined_plot(
        token_diffs, ranks, token_id, token_str, output_path, vocab_size
    )
