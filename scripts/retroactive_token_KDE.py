...
def tokenize_input(token_string: str, tokenizer_id: str) -> List[Tuple[int, str]]:
    ...
    for i, tid in enumerate(token_ids):
        ...
        print(
            f"  Token {i+1}: ID={tid}, decoded={repr(token_str)}, raw={repr(raw_token)}"
        )
...
def extract_token_diffs(
    diff: torch.Tensor, mask: torch.Tensor, token_id: int
) -> np.ndarray:
    """
...
def compute_token_ranks(
    diff: torch.Tensor, mask: torch.Tensor, token_id: int, exclude_zeros: bool = False
        ...
        for pos_idx in range(num_positions):
            if valid_mask[sample_idx, pos_idx]:
                ...
                if exclude_zeros:
                    ...
                    rank = (sorted_indices == token_id).nonzero(as_tuple=True)[
                        0
                    ].item() + 1  # 1-indexed
    ...
    avg_vocab = (
        int(np.mean(effective_vocab_sizes)) if effective_vocab_sizes else vocab_size
    )
...
def generate_combined_plot(
    ...
    sns.kdeplot(
        token_diffs, ax=ax1, fill=True, alpha=0.4, linewidth=2, color="steelblue"
    )
    ax1.axvline(
    ...
    ax1.set_title(
        f"Logit Diff (All)\nN={diff_stats['count']:,} | Std={diff_stats['std']:.6f}",
        fontsize=10,
    )
    ...
    ax2.set_title(
        f"Rank (All Vocab)\nN={rank_stats['count']:,} | Vocab={vocab_size:,}",
        fontsize=10,
    )
    ...
    if len(token_diffs_nonzero) > 1:
        ...
        ax3.set_title(
            f"Logit Diff (Excl. Zeros)\nN={diff_stats_nz['count']:,} | Std={diff_stats_nz['std']:.6f}",
            fontsize=10,
        )
    ...
    if len(ranks_nonzero) > 1:
        ...
        sns.kdeplot(
            ranks_nonzero, ax=ax4, fill=True, alpha=0.4, linewidth=2, color="coral"
        )
        ax4.axvline(
            x=rank_stats_nz["mean"],
            color="green",
            linestyle=":",
            linewidth=1.5,
            alpha=0.7,
        ...
        ax4.set_title(
            f"Rank (Non-zero Vocab)\nN={rank_stats_nz['count']:,} | Vocab~{vocab_size_nonzero:,}",
            fontsize=10,
        )
    ...
    safe_display = (
        repr(token_str) if len(token_str) < 20 else repr(token_str[:17] + "...")
    )
...
def analyze_token(
    ...
    print(
        f"  Non-zero positions: {len(token_diffs_nonzero):,} (avg non-zero vocab: ~{vocab_size_nonzero:,})"
    )
    ...
    if len(ranks_nonzero) > 0:
        ...
        print(
            f"    Mean: {rank_stats_nz['mean']:.1f}, Median: {rank_stats_nz['median']:.1f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default=".", help="Directory to save output plots"
    )
