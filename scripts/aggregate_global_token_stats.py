...
from diffing.methods.logit_diff_topk_occurring.plots import plot_global_token_scatter
...
def aggregate_token_stats(stats_files: List[Path]) -> Dict[str, Any]:
    ...
    for stats in all_stats:
        for token_entry in stats["global_token_stats"]:
            ...
            # Accumulate values
            token_stats_by_id[token_id]["sum_logit_diff"] += token_entry[
                "sum_logit_diff"
            ]
            token_stats_by_id[token_id]["count_positive"] += token_entry[
                "count_positive"
            ]

    # Convert back to sorted list by token_id
    combined_token_stats = sorted(
        token_stats_by_id.values(), key=lambda x: x["token_id"]
    )
...
def main():
    ...
    stats_files = sorted(
        [
            f
            for f in folder.glob("*global_token_stats.json")
    ...
    top_by_count = sorted(
        combined["global_token_stats"], key=lambda x: x["count_positive"], reverse=True
    )[:100]

    for i, t in enumerate(top_by_count, 1):
        token_display = repr(t["token"])[:28]
        print(
            f"{i:<6} {token_display:<30} {t['count_positive']:>18} {t['sum_logit_diff']:>18.2f}"
        )
    ...
    top_by_sum = sorted(
        combined["global_token_stats"], key=lambda x: x["sum_logit_diff"], reverse=True
    )[:100]
    ...
    config_path = (
        ...
        / "diffing"
        / "method"
        / "logit_diff_topk_occurring.yaml"
    )
